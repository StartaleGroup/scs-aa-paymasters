...
MY_ADDRESS="$(cast rpc eth_accounts | tail -n 1 | tr -d '[]"')"
ONE_TIME_SIGNER_ADDRESS="0x$(cat deployment.json | jq --raw-output '.signerAddress')"
GAS_COST="0x$(printf '%x' $(($(cat deployment.json | jq --raw-output '.gasPrice') * $(cat deployment.json | jq --raw-output '.gasLimit'))))"
TRANSACTION="0x$(cat deployment.json | jq --raw-output '.transaction')"
DEPLOYER_ADDRESS="0x$(cat deployment.json | jq --raw-output '.address')"
