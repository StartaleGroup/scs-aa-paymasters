...
import {PimlicoBase} from "./PimlicoBase.sol";
import {PimlicoMultiSigner} from "./PimlicoMultiSigner.sol";
...
abstract contract PimlicoERC20Base is Ownable, PimlicoBase, PimlicoMultiSigner {
    /*´:°•.°+.*•´.*:˚.°*.˚•´.°:°•.°•.*•´.*:˚.°*.˚•´.°:°•.°+.*•´.*:*/
    /*                       CUSTOM ERRORS                        */
    /*.•°:°.´+˚.*°.˚:*.´•*.+°.•°:´*.´•*.•°.•°:°.´:•˚°.*°.˚:*.´+°.•*/
    ...
    constructor(address _entryPoint, address _owner, address[] memory _signers)
        PimlicoBase(_entryPoint, _owner)
        PimlicoMultiSigner(_signers)
    {
        treasury = _owner;
    }
